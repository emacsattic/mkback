...
;; Time-stamp: <2007-12-06 10:12:58 deego>
...
(defun mkback-utils-unslash (dir)
  ;; (expand-file-name "" dir) used to work, but doesn't work for
  ;; tramp'ed files, so we work around the bug.
;;   (let 
;;       ((dir1 (expand-file-name "" dir)))
;;     ;; dir1 should already be good, barring emacs bugs as occur when
;;     ;; dir is a tramp name.
;;     (while (string-match "^\\(.*\\)/$" dir1)
;;       (setq dir1 (match-string 1 dir1)))
;;     dir1))
...
"
  ...
  (if (and mkback-max-depth (> depth mkback-max-depth))
    ...
    (progn
      ...
      (let* (
	     ;;(dir-unslashed (expand-file-name "" dir))
	     (dir-unslashed (mkback-utils-unslash dir))
...
(defvar mkback-recursive-p nil
  "See doc of mkback

If mkback-recursive-p is non-nil, will descend directories too!
If mkback-recursive-p is 'all, will descend LINKS too!  The latter
may be a very bad idea, since it can lead to infinite recursion,
depending on your link structure. ")


;;;###autoload
(defun mkback-recursive (&rest args)
  (interactive)
  (let ((mkback-recursive-p t))
    (apply 'mkback args)))
...
Instead of customizing `mkback-recursive-p', it might just be better
to use the function mkback-recursive.
"
  (interactive "F")
  (unless file 
  ...
  (if (listp file)
	...
	(cond
	 ((not mkback-recursive-p)
	  (error 
	   "Use M-x mkback-recursive.  This is a directory, skipping:  %S" file))
	 ;;((file-symlink-p file)
	 ;;(error 
	 ;;"This DIR is a SYMLINK, skipping: %S" file))
	 (t
	  (mapcar #'mkback-no-errors 
		  (mkback-directory-files file)))))
       (t
	(mkback-withit
...
(defun mkback-directory-files (dir)
  "Return all subfiles of the file.  Whether they be directories or
  filenames.  But don't return . and ...  Return full filenames. 

 Also remove backup/ subdirectory, if any... 
  Also remove symlinks TO directories. 
"
  (remove-if 
   (lambda (f)
     (and (file-exists-p f)
   ...
   (mapcar 
    ...
    (remove mkback-dir (remove "." (remove ".." (directory-files
						 dir)))))))
...
(defun mkback-install-for-eshell ()
  ...
  (defalias 'eshell/mkback-recursive 'mkback-recursive)
  )
...
(defmacro mkback-recursive-from-batch (&rest files)
  "The files get passed to emacs as symbols.. we need to simply format
them.."
  `(mkback-recursive
    (mkback-symbols-to-strings (quote ,files))))
...
(defmacro mkback-recursive-from-batch-doit (&rest files)
  "The files get passed to emacs as symbols.. we need to simply format
them.."
  `(let ((mkback-interactivity -100))
     (mkback-recursive
